...
@section('content')
    <div class="container">
        ...
        <div class="row">
            <div class="col-sm-12">
                <form id="product-form" class="product-form" action="{{ route('adminroles.store') }}" method="POST"
                            ...
                            <div class="row">
                                ...
                                @foreach ($permissions as $permission)
                                    <div class="form-section1 col-sm-4">
                                        <input type="checkbox" id="permission-{{ $permission->id }}" name="permissions[]" {{ old('permissions') ? 'checked' : '' }} value="{{ $permission->id }}">
                                        <label class="form-label" for="permission-{{ $permission->id }}" title="{{ $permission->description }}" >{{ $permission->name }}</label>
                                    </div>
                                @endforeach
